...
public class DownloadFileInternetExplorerBrowser {
	...
	public static void main(String[] args) {
		 ...
		 *  { Cookie httpClientCookie = new
		 * Cookie(seleniumCookie.getDomain(), seleniumCookie.getName(),
		 * seleniumCookie.getValue(), seleniumCookie.getPath(),
		 * seleniumCookie.getExpiry(), seleniumCookie.isSecure());
		 * cookieState.addCookie(httpClientCookie); } HttpMethod getFileRequest = new
		 * GetMethod(fileToDownloadAbsoluteLocation); boolean followRedirects = true;
		 * getFileRequest.setFollowRedirects(followRedirects);
		 * hClient.executeMethod(getFileRequest); String downloadPath =
		 * System.getProperty("user.dir")+"\\downloads"; File downloadedFile = new
		 * File(downloadPath+"\\DownloadDemo-master.zip");
		 * FileUtils.copyInputStreamToFile(getFileRequest.getResponseBodyAsStream(),
		 * downloadedFile); getFileRequest.releaseConnection();
		 * 
